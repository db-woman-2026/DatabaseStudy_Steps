...
마지막 날에는 리뷰를 배열에 추가하고 aggregation 파이프라인으로 재고·카테고리·리뷰 보고서를 만듭니다. 이어서 생성, 조회, 검색, 수정, 삭제를 처음부터 끝까지 수행합니다. 평가는 명령 암기보다 데이터 모델과 검증 절차를 설명하는 데 맞춥니다.
...
| 01:00~02:00 | 2교시 | 파이프라인 사고, `$match`, `$project`, 전체 재고 | 스테이지 추적표 |
| 02:00~03:00 | 3교시 | `$unwind`, category/review `$group` | 두 집계 보고서 |
| 03:00~04:00 | 4교시 | 빈 배열·통계 의미·파이프라인 확장 | 포함/제외 보고서 |
...
# 2교시. Aggregation 파이프라인 사고와 전체 재고 — 60분

## 2-1. 파이프라인은 변환 단계의 배열 — 10분
...
최종 seed나 삭제 전에 ledger, 명령 출력, 설계 회고를 저장합니다. 실제 DB 문서만 산출물로 두면 seed 후 사라집니다.
...
주의: step-8의 books_course와 step-5 publisher seed가 같은 데이터베이스에 존재하더라도 학습 단계 간 데이터 생명주기를 결합하는 것은 신중해야 합니다. mini project에서는 전용 publisher 컬렉션을 함께 준비합니다.
...
2. source of truth, snapshot, unbounded 배열을 각각 예로 설명합니다. (4점)

## B. CRUD — 12점

3. book add의 입력 validation, insertedId, 사후 get을 설명합니다. (3점)
4. search 필터의 OR/AND와 regex escape를 설명합니다. (3점)
5. stock/category/review update 연산자와 건수 검증을 설명합니다. (3점)
6. delete 사전 조회·confirm·deletedCount·사후 검증을 설명합니다. (3점)

## C. Aggregation — 12점

7. category 파이프라인의 스테이지별 문서 수/shape를 설명합니다. (4점)
...
- 25~34점: 6~8일차 종합 시나리오와 파이프라인 반복
- 0~24점: 모델/CRUD/aggregation 중 기초가 약한 날부터 재학습
...
이 과정을 완료했다는 것은 모든 SQL과 MongoDB 연산자를 외웠다는 뜻이 아닙니다. 새로운 요구에서 저장 단위, 식별자, 관계, 조건, 변경 범위, 검증 증거를 스스로 질문하고 안전하게 실험할 수 있다는 뜻입니다.
