# Step 1. Database Fundamentals와 RDBMS 기초 — 6시간 상세 강의

이 문서는 1일차 360분 수업을 설명·시연·안내 실습·독립 실습까지 그대로 진행할 수 있게 구성합니다. 오늘은 SQLite로 database 파일 하나와 `members` 테이블 하나를 만들며, 이후 7일 동안 반복할 안전한 데이터 작업 습관을 세웁니다.

## 0. 오늘의 핵심 질문

수업이 끝났을 때 다음 질문에 자신의 말과 실행 결과로 답할 수 있어야 합니다.

1. 단순 JSON 파일 대신 DBMS를 사용하는 이유는 무엇인가?
2. table, row, column, schema, primary key는 실제 회원 데이터에서 무엇인가?
3. 데이터 규칙을 JavaScript뿐 아니라 database 제약조건에도 두는 이유는 무엇인가?
4. 입력값을 SQL 문자열에 붙이지 않고 placeholder로 전달하는 이유는 무엇인가?
...
- data, database, DBMS, RDBMS, SQLite를 구분합니다.
- table, row, column, schema, instance, primary key를 설명합니다.
...
- `CREATE TABLE`로 schema를 정의합니다.
- prepared statement와 placeholder로 여러 행을 생성합니다.
...
- `npm start` 기준 실행 결과
- `members` schema를 자신의 말로 설명한 표
- 정상 INSERT 1회와 `UNIQUE` 실패 1회의 관찰 기록
- `phone` 또는 `membership_level`을 추가한 확장 schema
...
다른 SQLite 파일, 다른 프로젝트 파일, 실제 서비스 database를 사용하지 않습니다. 초기화 코드를 수정할 때 경로를 먼저 출력하고 현재 단계 파일인지 확인합니다.
...
| 01:00~02:00 | 2교시 | table/row/column/schema/key | 회원 데이터 표와 키 선택 근거 |
| 02:00~03:00 | 3교시 | SQLite 파일, 연결, 초기화, PRAGMA | 기준 코드 흐름도와 안전 경계 확인 |
| 03:00~04:00 | 4교시 | CREATE TABLE, 타입, 제약조건 | schema 해부와 실패 예제 |
...
| 용어 | 오늘의 예 |
| --- | --- |
| data | `"김민지"`, `"minji@example.com"` |
| database | 회원 행들이 저장된 `library-step-1.sqlite`의 논리 데이터 |
| DBMS | SQLite |
| application | `index.js` |
| query/command | `SELECT`, `INSERT` 등 SQL |
...
한 개체 또는 사건 한 건입니다. `members`에서는 회원 한 명, 이후 `loans`에서는 대출 사건 한 번이 row입니다.

### column

모든 행이 공유하는 속성과 규칙입니다. 이름뿐 아니라 타입, 필수 여부, 고유성, 기본값이 schema에 포함됩니다.
...
schema가 동일해도 INSERT/DELETE에 따라 state는 계속 변합니다.

## 2-3. 좋은 column 설계 — 10분

다음 회원 객체를 table column으로 옮깁니다.
...
오늘은 `id`를 primary key로 선택하고 email에는 `UNIQUE`를 둡니다. 식별과 업무 고유성의 책임을 나눕니다.
...
- [ ] 한 row가 무엇을 의미하는지 설명했습니다.
- [ ] schema와 현재 data state를 구분했습니다.
...
- 실제 서비스에서 시작할 때 database를 삭제하는 코드를 사용하지 않습니다.
...
`DatabaseSync`는 동기 API입니다. SQL 실행이 끝날 때까지 다음 코드로 넘어가지 않아 첫 학습에서 흐름을 보기 쉽습니다. 2일차부터 사용할 foreign key 검사를 연결 직후 켭니다. SQLite는 연결마다 이 설정이 필요합니다.
...
`members`라는 table schema를 만듭니다. 같은 이름의 table이 이미 있으면 오류가 납니다. 오늘은 파일을 매번 새로 만들기 때문에 `IF NOT EXISTS`가 필요하지 않습니다. 오류를 숨기지 않고 초기화 흐름이 잘못됐을 때 발견하기 위한 선택입니다.
...
빈 이름은 현재 schema에서 저장됩니다. `NOT NULL`이 빈 문자열을 막는다고 오해하지 않습니다. 이 관찰을 바탕으로 다음 확장 제약을 토론합니다.

```sql
name TEXT NOT NULL CHECK (length(trim(name)) > 0)
```

## 4-5. 다른 도메인 schema — 10분

다음 요구사항을 SQL column으로 바꿉니다.
...
이름에 작은따옴표가 들어가면 SQL이 깨질 수 있고, 신뢰할 수 없는 입력이 SQL 구조로 해석될 위험이 있습니다. placeholder는 데이터와 명령 구조를 분리합니다.
...
`member`는 `[name, email]`이고 `...member`는 두 인자로 펼쳐집니다. 배열 순서와 SQL placeholder 순서가 같아야 합니다.
...
placeholder를 사용하므로 작은따옴표가 SQL 종료 기호로 오해되지 않고 값으로 저장되는지 확인합니다.
...
SQL table 자체에는 화면 표시 순서가 보장되지 않습니다. 순서가 요구사항이면 `ORDER BY`를 명시합니다.
...
도서가 아닌 다음 중 하나의 table을 종이에 설계합니다.
...
짝과 schema를 바꿔 읽고, 어떤 잘못된 값을 막는지 설명합니다.
...
3. 내일 여러 table로 나누면 어떤 중복을 줄일 수 있을까?
...
동일한 시각이 있을 수 있어 오늘 데이터에서는 자동 ID를 가입 순서 대용으로 관찰하지만, 일반적으로 업무 순서를 정확히 표현하는 column을 사용합니다.
...
4. 같은 email을 두 번 넣고 오류 메시지에서 table과 column 이름을 찾습니다.
...
14. products table을 실제로 추가하고 정상·CHECK 실패 INSERT를 각각 실행합니다.
...
- 다른 database 파일에 연결하지 않았는지 `databasePath`를 출력합니다.
- CREATE TABLE 오류를 위에서 놓치지 않았는지 첫 오류부터 읽습니다.

## `UNIQUE constraint failed`

오류 column이 `members.email`인지 읽습니다. 기준 배열에 같은 email이 두 번 있는지 확인합니다. 오류를 피하려고 UNIQUE를 제거하지 않습니다.

## `NOT NULL constraint failed`

placeholder 순서와 전달한 값 개수를 확인합니다. `name`, `email` 중 어떤 필드가 NULL인지 오류 메시지를 읽습니다.
...
1. database와 DBMS의 차이를 오늘 파일과 제품 이름으로 설명하세요. (2점)
2. schema와 현재 state의 차이를 설명하세요. (2점)
3. primary key에 이름보다 자동 ID가 적합한 이유 두 가지를 적으세요. (2점)
4. `NOT NULL`과 `UNIQUE`가 각각 막는 오류를 적으세요. (2점)
5. 빈 문자열이 NOT NULL을 통과할 수 있는 이유를 적으세요. (2점)
6. placeholder 두 개와 `run()` 인자 순서가 다르면 어떤 문제가 생기나요? (2점)
...
6. 이름이 email column으로 가는 등 잘못된 위치에 저장되거나 제약 오류가 납니다.
7. 실제 생성된 행의 식별자를 확인합니다.
8. schema와 실제 행을 각각 검증합니다.
...
- 13~16점: key·constraint·prepared statement 실습 한 번 반복
- 9~12점: schema 표와 실패 실험을 강사와 다시 수행
...
다음을 모두 수행하면 1일차 핵심 경로를 완료한 것입니다.

- [ ] `step-1` 브랜치에서 기준 코드를 실행했습니다.
- [ ] database, DBMS, RDBMS, SQLite를 구분해 설명했습니다.
- [ ] table, row, column, schema, primary key를 회원 데이터에 연결했습니다.
- [ ] 현재 단계 SQLite 파일의 생성과 초기화를 확인했습니다.
- [ ] CREATE TABLE의 각 column과 제약조건을 설명했습니다.
- [ ] prepared statement로 회원을 한 명 이상 추가했습니다.
- [ ] `lastInsertRowid`와 전체 조회로 생성을 검증했습니다.
- [ ] UNIQUE 또는 NOT NULL 실패를 관찰하고 이유를 설명했습니다.
- [ ] PRAGMA로 schema를 확인했습니다.
- [ ] 조건 SELECT를 두 개 이상 실행했습니다.
- [ ] 확장 schema 하나를 설계하거나 구현했습니다.
...
5. products table 추가와 실패 입력 매트릭스 실행

## 다음 단계 연결

오늘은 모든 회원 정보를 table 하나에 저장했습니다. 2일차에는 저자, 도서, 회원, 대출을 한 table에 모두 넣으면 생기는 중복과 이상을 관찰하고 네 table로 나눕니다. 다음 질문을 한 문장으로 적어옵니다.
