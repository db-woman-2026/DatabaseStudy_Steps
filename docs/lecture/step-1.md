...
Step 1에는 SQLite 데이터베이스 파일과 `members` 테이블을 만듭니다. 데이터 생성 전 확인, 제약조건 오류 읽기, 저장 결과 조회를 이후 실습에서도 같은 순서로 반복합니다.
...
- 아래에서 `index.js` 전체를 입력한 뒤 문법 검사와 실행을 진행합니다.

### 3.3 파일 안전 범위

현재 단계 코드가 삭제할 수 있는 파일은 다음 하나뿐입니다.
...
| 검색 | 배열 전체 순회 가능 | 인덱스와 조건 조회 사용 |
| 규칙 | 코드의 `if`로 검사 가능 | 모든 저장 경로의 최종 제약 필요 |
| 동시성 | 한 명이 실행 | 같은 email 동시 등록 충돌 가능 |
| 복구 | 백업 파일 복사 | 일관된 상태와 복구 절차 필요 |

## 1-2. 개념 설명
데이터베이스는 일정한 구조와 규칙으로 관리되는 데이터 모음입니다. DBMS는 애플리케이션의 요청을 받아 저장·조회·변경·제약·동시성·복구를 담당하는 소프트웨어입니다.
...
## 1-3. 분류 활동
다음 사례에서 단순 파일과 DBMS 중 하나를 선택하고 이유를 한 줄로 적습니다. 정답은 하나가 아닐 수 있으며 조건을 함께 말해야 합니다.
...
- 주문·고객·회원처럼 검색, 관계, 규칙, 동시 변경 요구가 있으면 DBMS를 먼저 검토합니다.
...
이 예제에서는 `id`를 기본 키로 선택하고 email에는 `UNIQUE`를 둡니다. 식별과 업무 고유성의 책임을 나눕니다.
...
# 3. SQLite 파일과 기준 코드 흐름
## 3-1. 전체 코드를 먼저 지도처럼 읽기
...
이 예제에서는 매번 같은 세 회원으로 시작해야 결과를 함께 비교할 수 있습니다. 그래서 현재 단계 파일만 삭제하고 다시 만듭니다.
...
`DatabaseSync`는 동기 API입니다. SQL 실행이 끝날 때까지 다음 코드로 넘어가지 않아 처음 실행할 때 흐름을 확인하기 쉽습니다. 외래 키 검사는 연결 직후 켭니다. SQLite는 연결마다 이 설정이 필요합니다.
...
`members`라는 테이블 스키마를 만듭니다. 같은 이름의 테이블이 이미 있으면 오류가 납니다. 파일을 매번 새로 만들기 때문에 `IF NOT EXISTS`가 필요하지 않습니다. 오류를 숨기지 않고 초기화 흐름이 잘못됐을 때 발견하기 위한 선택입니다.
...
`AUTOINCREMENT`를 모든 테이블에 무조건 사용해야 한다는 뜻은 아닙니다. 여기서는 생성 ID 흐름을 명확히 보기 위해 사용합니다.
...
Step 3의 트랜잭션으로 연결되는 질문입니다. 현재 단계 기준 코드는 실행마다 파일을 다시 만들기 때문에 다음 실행에서 상태가 초기화됩니다.
...
스키마를 다시 읽고, 어떤 잘못된 값을 막는지 설명합니다.

## 6-5. 기준 코드 복원과 최종 실행
실험 중 임시 실패 코드가 남아 있다면 제거합니다. 개인 확장은 별도 메모나 커밋으로 보관하고 기준 실행이 성공하는지 확인합니다.
...
동일한 시각이 있을 수 있어 현재 단계 데이터에서는 자동 ID를 가입 순서 대용으로 관찰하지만, 일반적으로 업무 순서를 정확히 표현하는 열을 사용합니다.
...
`Minji@example.com`과 `minji@example.com`을 같은 값으로 볼지 정책이 필요합니다. 입력을 소문자로 정규화하거나 collation/함수 기반 고유성 등 제품별 방법을 검토할 수 있습니다. 현재 `UNIQUE`가 실제로 어떤 값을 다르게 보는지 확인합니다.
...
- 트랜잭션은 `BEGIN → 여러 INSERT → COMMIT`, 오류 시 `ROLLBACK`으로 처리합니다.
...
현재 단계 코드는 같은 상태를 재현하려고 매 실행마다 현재 단계 파일을 삭제합니다. 영속성 기능이 없다는 뜻이 아닙니다. 초기화 코드를 제거하면 파일 상태를 유지할 수 있지만, 이 예제에서는 동일한 기준 상태에서 결과를 비교합니다.

## ID가 예상과 다릅니다

현재 단계 파일이 정말 초기화됐는지, 임시 INSERT를 추가했는지 확인합니다. 일반 시스템에서 ID 연속성은 업무 규칙이 아니며 ID의 목적은 고유 식별입니다.

## `console.table()`에서 시각이 예상과 다릅니다
...
1. 데이터베이스와 DBMS의 차이를 현재 단계 파일과 제품 이름으로 설명하세요.
2. 스키마와 현재 state의 차이를 설명하세요.
3. 기본 키에 이름보다 자동 ID가 적합한 이유 두 가지를 적으세요.
4. `NOT NULL`과 `UNIQUE`가 각각 막는 오류를 적으세요.
5. 빈 문자열이 NOT NULL을 통과할 수 있는 이유를 적으세요.
6. 플레이스홀더 두 개와 `run()` 인자 순서가 다르면 어떤 문제가 생기나요?
7. `lastInsertRowid`를 확인하는 이유는 무엇인가요?
8. `PRAGMA table_info`와 `SELECT`가 각각 무엇을 검증하나요?
9. 현재 단계 파일만 삭제해야 하는 이유를 적으세요.
10. 새 회원 생성이 성공했음을 증명할 두 가지 결과를 적으세요.
...
6. 이름이 email 열로 가는 등 잘못된 위치에 저장되거나 제약 오류가 납니다.
...
회원 정보를 한 테이블에 저장한 방식에서 저자, 도서, 회원, 대출 관계로 범위를 넓히면 중복과 이상이 생깁니다. 다음 질문의 답을 한 문장으로 적습니다.
