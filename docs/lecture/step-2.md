...
1일차에는 회원 데이터를 `members` 테이블 하나에 저장했습니다. 이번에는 저자·도서·회원·대출을 각 테이블로 나누고 기본 키와 외래 키로 연결합니다. 테이블을 나눈 이유와 JOIN 결과의 한 행이 무엇을 뜻하는지 설명할 수 있어야 합니다.
...
| 01:00~02:00 | 2교시 | 키, cardinality, N:M 해소, 정규화 직관 | 4-테이블 ERD와 키 근거 |
| 02:00~03:00 | 3교시 | DDL, 제약조건, 외래 키 검사 | 스키마 해부와 실패 실험 |
...
## 2-3. natural 키와 surrogate 키 — 10분

| 테이블 | 자연키 후보 | 대체키 | 오늘 선택 |
| --- | --- | --- | --- |
...
MongoDB 단계에서는 `publisherCode` 같은 참조가 실제 출판사 문서를 가리키는지 DB가 자동으로 검사하지 않는 예를 봅니다. 오늘 외래 키가 주는 보장과 대비하여 기록합니다.
...
모든 스키마/데이터 검증 뒤 `database.close()`를 호출합니다. 오늘도 실행 과정 전체가 현재 단계 파일 하나의 생명주기입니다.
...
이름이 교육용 스키마에서 UNIQUE이므로 특정 저자를 고르지만 실제 모델에서는 저자 ID를 식별 조건으로 쓰는 편이 안정적입니다.
...
1. 큰 대출 테이블의 수정 이상을 예로 설명하세요. (2점)
2. 기본 키와 외래 키의 역할 차이를 설명하세요. (2점)
...
6. 부모 행를 먼저 INSERT해야 하는 이유를 쓰세요. (2점)
...
- 9~12점: 원본 테이블을 그린 뒤 손으로 JOIN 결과 만들기
- 0~8점: 1교시 큰 테이블 이상부터 다시 모델링
