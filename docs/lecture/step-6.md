...
오늘은 Create와 Read로 state를 만들고 확인했습니다. 7일차에는 복합 search, 부분 Update, 안전 Delete를 추가합니다. 특히 `matchedCount`와 `modifiedCount`가 다른 이유, 같은 category를 두 번 추가했을 때 왜 두 번째 변경이 0인지 관찰합니다.
