...
MongoDB 서버와 `.env`의 강의 전용 데이터베이스를 확인합니다. 오늘 사용할 컬렉션은 `books_course`입니다. step-6부터 step-8까지 같은 컬렉션을 이어 쓰므로 `seed`를 실행하면 이전 실습 변경이 사라진다는 점을 항상 알립니다.
...
| 시간 | 블록 | 내용 | 필수 결과 |
| --- | --- | --- | --- |
| 00:00~01:00 | 1교시 | CLI, process.argv, 명령 routing, seed | 명령/상태 표 |
| 01:00~02:00 | 2교시 | 입력 검증·정규화·문서 변환 | 변환 파이프라인과 오류 사례 |
| 02:00~03:00 | 3교시 | 단위 테스트와 경계값 | test matrix와 추가 test |
| 03:00~04:00 | 4교시 | Create, insertOne, unique | 생성·중복 결과 검증 |
| 04:00~05:00 | 5교시 | Read, find, 필터, 프로젝션, 정렬 | 전체·category 목록 |
...
현재 step-6에는 Update/Delete가 없습니다. 7일차에 명령 map과 변경 기능을 추가합니다.
...
# 5교시. Read 목록: find, 필터, 프로젝션, 정렬 — 60분
...
첫 명령은 모든 book, 두 번째는 categories에 데이터베이스가 포함된 book만 반환합니다.
...
알 수 없는 명령은 help를 보여주고 비정상 상태로 종료합니다. 7일차에는 명령 map으로 확장됩니다.
...
11. 데이터베이스, beginner, mongodb, unknown category 결과를 비교합니다.
...
- DB 통합 test는 강의 전용 test 데이터베이스/컬렉션, 독립 seed, cleanup, 병렬 실행 격리가 필요합니다.
...
1. `process.argv.slice(2)`에서 명령과 args를 어떻게 나누는지 설명하세요. (2점)
