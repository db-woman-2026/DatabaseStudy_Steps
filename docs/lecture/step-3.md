# Step 3. SQL 조회·조작·트랜잭션 — 6시간 상세 강의

1~2일차에 만든 관계형 schema를 실제 업무 질문에 사용합니다. 오늘은 `SELECT`를 단순 전체 조회에서 조건·정렬·집계로 확장하고, `UPDATE`와 `DELETE`를 안전하게 실행합니다. 마지막에는 재고 감소와 대출 생성을 하나의 트랜잭션으로 묶어 “일부만 성공한 상태”를 막습니다.

## 0. 오늘의 핵심 질문

1. 필요한 행과 column만 정확히 선택하려면 SQL의 각 절을 어떤 순서로 생각해야 하는가?
...
- placeholder를 사용한 filter와 다중 정렬을 작성합니다.
...
오늘 파일은 `data/library-step-3.sqlite`이며 실행마다 다음 기준 상태로 초기화됩니다.
...
| 시간 | 블록 | 핵심 내용 | 필수 결과 |
| --- | --- | --- | --- |
| 00:00~01:00 | 1교시 | SELECT pipeline, WHERE, NULL, LIKE | 조건 조회 4개 |
| 01:00~02:00 | 2교시 | ORDER BY, LIMIT, 집계 함수 | 정렬·전체 요약 결과 |
| 02:00~03:00 | 3교시 | GROUP BY, HAVING, LEFT JOIN 집계 | 저자별·연도별 보고서 |
| 03:00~04:00 | 4교시 | UPDATE, DELETE, 안전 루프 | 변경 전후 검증표 |
| 04:00~05:00 | 5교시 | transaction, ACID, 대출 함수 | COMMIT/ROLLBACK 상태 비교 |
| 05:00~06:00 | 6교시 | 반납 transaction, 종합 쿼리, 평가 | 구현안과 RDBMS 구간평가 |
...
오늘 쿼리는 이 관계를 유지한 상태에서 어떤 행을 읽고 바꿀지 결정합니다.

## 1-2. SELECT의 질문 네 가지 — 10분
...
3. 어떤 column을 보여주는가? — `SELECT title, ...`
...
사용자가 입력한 `%`와 `_`를 문자 그대로 찾고 싶다면 ESCAPE 정책이 필요합니다. 오늘은 패턴의 의미를 이해하고 값을 placeholder로 전달합니다.
...
OFFSET이 매우 커지면 앞 행을 건너뛰는 비용이 생길 수 있습니다. 실제 대규모 페이지에서는 마지막 정렬 키를 조건으로 쓰는 keyset pagination을 검토하지만, 오늘은 LIMIT과 안정 정렬에 집중합니다.
...
COUNT(column)은 NULL을 세지 않습니다. 이를 이용할 수도 있지만 의도를 별칭으로 명확히 합니다.
...
- [ ] COUNT(*)와 COUNT(column)을 구분했습니다.

---

# 3교시. GROUP BY와 HAVING — 60분

## 3-1. 그룹 단위 먼저 정하기 — 10분
...
저자 이름이 같을 수 있는 현실 모델에서도 ID가 그룹 식별자입니다. name은 출력에 필요합니다. SQLite가 느슨하게 허용하는 GROUP BY 표현에 의존하지 않고, 집계하지 않은 출력 column을 그룹 기준에 명확히 둡니다.
...
저장 column이 없는 표시용 status 표현을 그룹 키로 사용합니다. 기준 데이터는 각 상태 1건입니다.
...
SQLite의 변경 카운트 동작은 설정과 실제 쓰기 방식에 따라 동일 값 대입을 변경으로 셀 수 있는 등 제품/API 차이가 있을 수 있습니다. 핵심은 카운트만으로 모든 원인을 단정하지 않고 사전·사후 조회와 함께 보는 것입니다.

## 4-4. 반납 완료 대출 DELETE — 10분
...
모든 대출 기록을 삭제합니다. 강의용 초기화라 해도 의도와 범위를 명시합니다.

## 4-6. soft delete 토론 — 10분
...
- 보존 기간 뒤 archive table로 이동
- `deleted_at`을 두는 soft delete

오늘 코드는 DELETE 동작을 학습하기 위한 예입니다. 기술 기능을 쓸 수 있다는 것과 업무에서 써야 한다는 것은 다릅니다.
...
member ID가 없으면 foreign key 오류가 발생합니다. 이때 앞에서 성공한 재고 감소도 ROLLBACK되어야 합니다.
...
# 6교시. ACID, 반납 transaction, 종합 평가 — 60분

## 6-1. ACID를 오늘 코드에 연결 — 15분
...
동시에 두 대출이 실행될 때 서로의 중간 변경이 어떻게 보일지 다룹니다. SQLite의 locking과 transaction 모드는 별도 심화 주제이며, 오늘은 조건부 UPDATE가 재고 음수를 막는 핵심을 봅니다.

### Durability — 지속성

COMMIT된 결과는 연결을 닫고 다시 열어도 database 파일에 남아야 합니다. 단, 오늘 코드는 다음 `npm start`에서 교육 목적으로 파일을 초기화합니다.
...
3. rollback됐음을 증명하려면 어떤 두 table을 조회해야 하는가?
...
step-3은 조회·조작에 집중하기 위해 여러 기준 행을 한 `database.exec()`에 준비합니다. 실제 사용자 입력을 SQL literal로 연결하라는 뜻이 아닙니다. 동적 입력은 계속 placeholder로 전달합니다.

## 7-2. published_year 추가

2일차 books에 `published_year INTEGER NOT NULL`이 추가됐습니다. 정렬·범위·그룹 실습을 위한 새 속성입니다.
...
같은 결과를 JOIN+GROUP BY로도 만들 수 있습니다. 오늘은 읽기 쉬운 쪽을 비교하고 실제 성능은 실행 계획과 데이터 규모로 검증해야 함을 이해합니다.
...
18. 재고 1 이하 도서에 보충 표시 column을 추가하는 UPDATE를 설계합니다.
...
- SELECT의 집계하지 않은 column이 GROUP BY에 있는지 확인합니다.
...
`%` 위치, 대소문자/collation, 실제 title 값을 확인합니다. 값은 placeholder로 전달합니다.
...
1. authors-books와 members-loans-books 관계를 그려 PK/FK를 표시합니다. (4점)
2. 재고 2 이상인 책의 제목·저자·재고를 최신 출판 연도순으로 조회합니다. (3점)
3. 저자별 도서 수와 총 재고를 도서 없는 저자까지 표시합니다. (3점)

## B. 변경 안전성 — 8점

4. member ID 2의 email을 바꾸는 사전 SELECT, UPDATE, 사후 SELECT를 작성합니다. (4점)
5. 반납 완료 대출을 삭제하기 전후 확인 절차와 changes 검사를 적습니다. (4점)

## C. 트랜잭션 — 8점

6. 대출 처리의 두 변경과 부분 성공 문제를 설명합니다. (3점)
7. stock 0 또는 없는 member에서 rollback 후 확인할 상태를 적습니다. (2점)
8. 반납 transaction의 단계와 실패 조건을 의사코드로 작성합니다. (3점)

## D. 설명 — 4점

9. WHERE와 HAVING 차이를 예로 설명합니다. (2점)
10. transaction이 잘못된 업무 로직을 자동으로 고치지 못하는 이유를 설명합니다. (2점)
...
- 26~30점: MongoDB 문서 모델 과정 진행
- 21~25점: GROUP BY/LEFT JOIN 또는 transaction 약점 한 영역 보충
- 15~20점: 3일차 핵심 경로 반복 후 재평가
- 0~14점: 2일차 관계 모델부터 결과 행을 손으로 추적
...
- [ ] ACID를 오늘 대출 사례에 연결해 설명했습니다.
- [ ] 반납 transaction을 구현하거나 상세 의사코드로 작성했습니다.
- [ ] 구간평가에서 21점 이상을 받았습니다.
...
3. 물리적 copy model의 대출 transaction 설계
4. `EXPLAIN QUERY PLAN`으로 조회 계획 관찰
5. SQLite transaction mode와 동시 쓰기 실험 설계

## 다음 단계 연결

3일 동안 관계형 모델은 데이터를 나누고 foreign key와 transaction으로 일관성을 지켰습니다. 4일차에는 같은 도서 데이터를 MongoDB의 중첩 document로 저장합니다. 다음 비교를 미리 적습니다.

> SQL에서 책·저자·재고를 여러 table과 column으로 다뤘다. 도서 상세 화면에서 항상 함께 읽는 값들을 한 document에 넣으면 무엇이 단순해지고, 무엇이 새 책임이 될까?
