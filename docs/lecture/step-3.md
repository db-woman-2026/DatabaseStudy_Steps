...
| 시간 | 블록 | 내용 | 필수 결과 |
| --- | --- | --- | --- |
| 00:00~01:00 | 1교시 | SELECT 파이프라인, WHERE, NULL, LIKE | 조건 조회 4개 |
...
사용자가 입력한 `%`와 `_`를 문자 그대로 찾고 싶다면 ESCAPE 정책이 필요합니다. 오늘은 패턴의 의미를 이해하고 값을 플레이스홀더로 전달합니다.
...
COMMIT된 결과는 연결을 닫고 다시 열어도 데이터베이스 파일에 남아야 합니다. 단, 오늘 코드는 다음 `npm.cmd start`에서 교육 목적으로 파일을 초기화합니다.
...
`npm.cmd start` 한 번 안에서도 여러 변경이 순서대로 일어납니다. 최종 출력만 보고 초기 상태로 오해하지 않도록 타임라인을 추적합니다.
...
매 `npm.cmd start`가 단계 파일을 초기화하고 seed를 다시 넣기 때문입니다. transaction의 지속성 실패가 아닙니다.
...
3일 동안 관계형 모델은 데이터를 나누고 외래 키와 transaction으로 일관성을 지켰습니다. 4일차에는 같은 도서 데이터를 MongoDB의 중첩 문서로 저장합니다. 다음 비교를 미리 적습니다.

> SQL에서 책·저자·재고를 여러 테이블과 열으로 다뤘다. 도서 상세 화면에서 항상 함께 읽는 값들을 한 문서에 넣으면 무엇이 단순해지고, 무엇이 새 책임이 될까?
