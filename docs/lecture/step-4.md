...
`step-4`부터 `mongodb` Node.js Driver가 dependency에 추가됩니다. `npm.cmd ci`는 lockfile에 고정된 버전을 설치합니다.
...
오늘은 author·inventory·categories를 한 문서에 넣었습니다. 5일차에는 항상 함께 읽는다는 이유만으로 포함을 결정하지 않고 변경 빈도, 공유, 크기, 일관성, 배열 성장까지 평가합니다. ISBN 고유성과 반복 검색을 위한 인덱스도 추가합니다.
