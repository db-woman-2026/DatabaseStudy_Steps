...
npm pkg set "scripts.check=node --check index.js && node --check lib/mongodb.js && node --check lib/sampleBooks.js"
```
...
로컬 MongoDB 서버가 필요합니다. 아래에서 소스 파일 전체를 입력한 뒤 `npm start`로 연결을 확인합니다. 연결에 실패해도 바로 코드를 바꾸지 않고 오류 진단 절의 순서대로 서버, URI, 네트워크, 인증, 데이터베이스 이름을 분리합니다.
