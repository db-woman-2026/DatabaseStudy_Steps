...
이 문서는 2~3일차의 선수 자료입니다. SQL 문법 목록을 외우는 대신, 요구사항을 테이블 관계로 바꾸고 그 관계를 조회·변경하는 흐름을 이해합니다.
...
제품이나 교재에 따라 `SELECT`를 DML에 포함하기도 합니다. 분류 자체보다 각 문장이 schema를 바꾸는지, 행을 읽는지, 현재 상태를 변경하는지를 구분하는 것이 중요합니다.
...
- super key: 행을 고유하게 구분할 수 있는 속성의 집합
- candidate key: 불필요한 속성을 뺀 최소 super key

회원 테이블에서 `id`, `email`이 각각 고유하다면 둘 다 candidate key가 될 수 있습니다.

### primary key와 alternate key

candidate key 중 대표로 선택한 것이 primary key이고, 선택되지 않은 고유 키는 alternate key라고 볼 수 있습니다. `id`를 PK로 선택하고 `email`에는 `UNIQUE`를 둘 수 있습니다.

### foreign key

다른 테이블의 candidate/primary key를 가리키는 column입니다.
...
foreign key는 단지 이름이 `_id`로 끝난다고 생기지 않습니다. `FOREIGN KEY ... REFERENCES ...` 제약을 정의하고 SQLite 연결에서 검사를 켜야 DBMS가 참조 무결성을 확인합니다.

### 복합키

두 개 이상의 column이 함께 행을 구분할 수도 있습니다.
...
N 쪽인 `books`가 `author_id` foreign key를 가집니다.
...
아직 책을 쓰지 않은 저자를 등록하려면 도서 column에 NULL을 허용하거나 가짜 도서를 만들어야 합니다.
...
이 과정에서는 정규형을 수학적으로 증명하기보다 “이 사실은 어느 대상에 속하는가?”, “한 곳만 바꿔도 일관성이 유지되는가?”를 질문합니다.
...
foreign key가 가리키는 부모 행을 삭제할 때의 정책도 설계 대상입니다.

- `RESTRICT`/`NO ACTION`: 참조 중이면 삭제 거부
- `CASCADE`: 부모 삭제 시 자식도 삭제
- `SET NULL`: 자식 foreign key를 NULL로 변경
...
문자열 연결은 따옴표가 포함된 정상 값도 깨뜨릴 수 있고, 입력이 SQL 구조로 해석되는 SQL injection 위험을 만듭니다. placeholder와 바인딩은 SQL 구조와 데이터를 분리합니다.
...
외부 결제 API까지 포함되면 database transaction 하나만으로 해결되지 않아 보상 작업이나 상태 머신 같은 추가 설계가 필요합니다.
...
1. `loans`가 단순 연결 테이블을 넘어 하나의 엔터티인 이유는 무엇인가요?
2. `authors`와 `books`에서 foreign key는 어느 테이블에 두는 것이 자연스러운가요?
...
- [ ] primary key와 foreign key를 구분할 수 있습니다.
