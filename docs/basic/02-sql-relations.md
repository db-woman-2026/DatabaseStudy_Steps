...
2~3일차 전에 읽을 자료입니다. SQL 문법을 나열하기보다 요구사항을 테이블 관계로 바꾸고 조회·변경하는 순서를 익힙니다.
...
제품이나 교재에 따라 `SELECT`를 DML에 포함하기도 합니다. 분류 자체보다 각 문장이 스키마를 바꾸는지, 행을 읽는지, 현재 상태를 변경하는지를 구분하는 것이 중요합니다.
