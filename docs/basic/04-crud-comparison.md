...
`updateMany()`는 위험한 메서드라서 사용하지 말아야 하는 것이 아니라, 요구사항이 여러 문서 변경일 때 쓰는 도구입니다. 다만 범위 검증이 더 중요합니다.
...
이 과정의 escape는 사용자가 입력한 `Node.js`의 점을 “임의의 한 문자”가 아니라 실제 점으로 검색하기 위한 것입니다. 대규모 부분 문자열 검색은 일반 B-tree 인덱스를 충분히 활용하지 못할 수 있으므로 실제 서비스에서는 text index 또는 검색 엔진 등 접근 패턴에 맞는 도구를 검토합니다.
