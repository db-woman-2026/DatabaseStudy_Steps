...
이 문서는 4~5일차의 선수 자료입니다. JavaScript 객체와 비슷해 보인다는 이유만으로 MongoDB를 “JSON 파일 저장소”로 이해하지 않고, DBMS로서 제공하는 문서 저장·조회·인덱스·원자적 변경을 구분합니다.
...
- String
- 32-bit/64-bit integer, double, decimal
- Boolean
- Date
- ObjectId
- Binary data
...
관련 collection을 묶는 이름 공간입니다. 이 과정에서는 `database_study_`로 시작하는 강의 전용 이름만 허용합니다.

### collection

같은 목적의 document 모음입니다. 관계형 데이터베이스의 table과 비교할 수 있지만, 모든 문서가 항상 완전히 같은 필드를 갖도록 자동 강제되는 것은 아닙니다.
...
문서의 키와 값입니다. table의 column과 비교할 수 있지만 문서마다 필드 존재 여부가 다를 수 있고 중첩 구조를 가질 수 있습니다.
...
같은 collection의 문서가 서로 다른 필드를 가질 수 있습니다.
...
“schema-less”보다는 “schema-flexible”이라고 이해하는 편이 정확합니다.
...
- 카테고리: 보통 도서당 몇 개로 제한되어 bounded array에 가깝습니다.
- 리뷰: 서비스 규모에 따라 계속 늘어날 수 있는 unbounded array입니다.
- 센서 측정값: 초당 계속 추가되므로 한 장치 문서에 영원히 넣으면 안 됩니다.

MongoDB 문서에는 크기 제한이 있고, 큰 배열은 읽기·쓰기 비용과 경합을 늘립니다. 무한히 커질 수 있는 데이터는 별도 collection, bucket, time-series 구조 등을 검토합니다.
...
- MongoDB가 일반 foreign key처럼 존재를 자동 보장하지 않습니다.
...
- 저자 요약과 재고는 도서에 포함
- 출판사는 별도 collection에 두고 code 참조
- categories는 작은 배열로 포함
- 리뷰가 크게 성장하면 별도 collection 검토
...
`.env`의 URI에는 인증 정보가 포함될 수 있으므로 커밋하지 않습니다. 강의에서는 실제 업무 database가 아니라 안전 접두사를 가진 전용 database만 사용합니다.
...
`find()`는 즉시 모든 문서를 배열로 돌려주는 것이 아니라 cursor를 반환합니다. cursor에 정렬, 제한, projection을 구성한 뒤 결과를 소비합니다.
...
같은 객체의 서로 다른 필드는 기본적으로 AND입니다. 위 조건은 database 카테고리이면서 재고가 2 이상인 도서입니다.
...
필요한 필드만 반환하면 네트워크 전송량과 애플리케이션이 다룰 데이터를 줄일 수 있습니다. 보통 포함 방식(1)과 제외 방식(0)을 한 projection에서 섞지 않지만 `_id` 제외는 예외적으로 함께 사용합니다.

projection은 저장된 문서를 바꾸지 않습니다. 조회 결과의 모양만 정합니다.

## 16. 인덱스

인덱스는 특정 필드 값과 문서 위치를 정렬된 구조로 관리해, 모든 문서를 검사하는 collection scan을 줄일 수 있습니다.
...
| 질문 | 관계형 모델 | 문서 모델 |
| --- | --- | --- |
| 기본 저장 단위 | row | document |
| 구조 정의 | table schema와 constraint | 문서 구조, 앱 검증, schema validation |
| 관계 표현 | foreign key | embedding 또는 reference |
| 함께 읽기 | JOIN | 한 문서 또는 `$lookup`/추가 조회 |
| 중첩 구조 | 관련 table로 분리 | 객체·배열로 자연스럽게 포함 |
...
- 댓글이 수십만 개까지 늘고 별도 신고·검색·페이지네이션이 필요하면 collection 분리를 검토합니다.

### 상품과 가격 이력

- 현재 가격은 상품 문서에 포함할 수 있습니다.
- 모든 가격 변경 이력은 계속 성장하므로 별도 collection이 자연스러울 수 있습니다.
...
2. collection이 table과 비슷하지만 완전히 같지 않은 이유는 무엇인가요?
3. 유연한 schema가 데이터 규칙이 필요 없다는 뜻이 아닌 이유는 무엇인가요?
4. `inventory.stock`만 수정할 때 상위 `inventory` 객체 전체를 `$set`하면 어떤 위험이 있나요?
5. categories와 reviews 배열의 성장 특성이 어떻게 다른가요?
6. 출판사를 참조했을 때 MongoDB가 foreign key처럼 자동 보장하지 않는 것은 무엇인가요?
7. unique index와 일반 검색 인덱스의 역할 차이를 설명하세요.
8. projection이 저장된 문서를 바꾸나요?

<details>
<summary>개념 확인 해설</summary>

1. JSON은 텍스트 교환 형식, JavaScript 객체는 런타임 메모리 값, BSON은 MongoDB가 다양한 타입과 함께 저장하는 이진 문서 형식입니다.
2. 문서마다 필드가 다를 수 있고 중첩 객체·배열을 직접 가질 수 있으며 관계를 foreign key로만 표현하지 않기 때문입니다.
3. 타입과 필드 위치가 제각각이면 조회·정렬·집계가 어려워지므로 애플리케이션 검증, schema validation, 인덱스 등이 필요합니다.
...
- [ ] server, database, collection, document, field를 계층으로 설명할 수 있습니다.
- [ ] JSON과 BSON의 차이를 한 가지 이상 설명할 수 있습니다.
- [ ] `_id`의 역할을 설명할 수 있습니다.
- [ ] 포함과 참조 선택 질문을 세 가지 이상 말할 수 있습니다.
- [ ] bounded array와 unbounded array의 예를 들 수 있습니다.
- [ ] 점 표기법으로 중첩 필드를 조회할 수 있습니다.
- [ ] cursor와 array가 같은 것이 아님을 알고 있습니다.
