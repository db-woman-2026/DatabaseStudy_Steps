...
MongoDB는 문서 저장·조회·인덱스·원자적 변경을 제공하는 DBMS입니다.
...
관련 컬렉션을 묶는 이름 공간입니다. 이 예제에서는 `database_study_`로 시작하는 전용 이름만 허용합니다.
...
한 저장 단위입니다. 중첩 객체와 배열을 포함할 수 있습니다. `books_course`에서 문서 하나는 도서 한 권을 나타냅니다.
...
배열로 표현할 수 있지만 성장 한계를 반드시 확인해야 합니다.
...
`.env`의 URI에는 인증 정보가 포함될 수 있으므로 커밋하지 않습니다. 실제 업무 데이터베이스가 아니라 안전 접두사를 가진 전용 데이터베이스만 사용합니다.
